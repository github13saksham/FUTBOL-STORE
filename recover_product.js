...
let files = {};
files['c:/users/saksham/downloads/futbol store/src/app/product/[id]/page.tsx'] = fs.readFileSync('product_base_clean.tsx', 'utf8');

for(let i=0; i<lines.length; i++) {
  const line = JSON.parse(lines[i]);
  if (line.step_index <= 176) continue;
  ...
  if (line.type === 'PLANNER_RESPONSE' && line.tool_calls) {
    line.tool_calls.forEach(tc => {
      ...
      file = file.replace(/^"|"$/g, '').toLowerCase().replace(/\\/g, '/');
      
      if (!file.includes('product/[id]/page.tsx')) return;
...
const finalContent = files['c:/users/saksham/downloads/futbol store/src/app/product/[id]/page.tsx'];
fs.writeFileSync('src/app/product/[id]/page.tsx', finalContent);
console.log('Successfully recovered product page with all organic changes!');
